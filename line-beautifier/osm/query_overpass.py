...
blacklist_highway = ['pedestrian', 'track', 'bus_guideway', 'escape', 'raceway',
                     ...
                     'stop', 'traffic_signals', 'footway', 'platform']
...
def build_query(bounds):
    ...
    blacklist = ''.join(['["highway"!="' + b + '"]' for b in blacklist_highway])
    blacklist = blacklist + ''.join(['["service"!="' + b + '"]' for b in blacklist_service])
    blacklist = blacklist + ''.join(['["access"!="' + b + '"]' for b in blacklist_access])
    ways = 'way[!"area"]["highway"]{blacklist}({y1},{x1},{y2},{x2});'
